...
interface ParsedMcpCliOptions extends Required<Pick<McpServerStartOptions, 'transport' | 'host' | 'port' | 'path'>> {
	help: boolean;
}

let consoleCaptureQueue: Promise<void> = Promise.resolve();

export function createFhirControllerMcpServer() {
			...
			content: [
				{
					...
					text: [
						...
						'Transports: stdio for local MCP clients, or Streamable HTTP for container deployments.',
...
export async function startMcpServer(options: McpServerStartOptions = {}) {
	const transport = options.transport ?? transportFromEnv() ?? 'stdio';
	if (transport === 'http') {
		await startHttpMcpServer(options);
		return;
	}

	const server = createFhirControllerMcpServer();
	await server.connect(new StdioServerTransport());
}

export async function startMcpServerFromCli(args: string[]) {
	const options = parseMcpCliOptions(args);
	if (options.help) {
		printMcpHelp();
		return;
	}
...
export function parseMcpPort(value: string | number | undefined): number {
...
async function startHttpMcpServer(options: McpServerStartOptions) {
	const host = options.host ?? process.env.FHIR_CONTROLLER_MCP_HOST ?? process.env.HOST ?? defaultHttpHost;
	const port = options.port ?? parseMcpPort(process.env.FHIR_CONTROLLER_MCP_PORT ?? process.env.MCP_PORT ?? process.env.PORT);
	const endpointPath = normalizeEndpointPath(options.path ?? process.env.FHIR_CONTROLLER_MCP_PATH ?? defaultHttpPath);
	const installSignalHandlers = options.installSignalHandlers ?? false;

	const httpServer = createServer(async (req, res) => {
		try {
			await handleHttpMcpRequest(req, res, endpointPath);
		} catch (error) {
	...
	console.error(`FHIR Controller MCP Streamable HTTP server listening on http://${displayHost(host)}:${port}${endpointPath}`);

	if (installSignalHandlers) {
		const shutdown = () => {
			console.error('Shutting down FHIR Controller MCP HTTP server...');
...
async function handleHttpMcpRequest(req: IncomingMessage, res: ServerResponse, endpointPath: string) {
	const requestUrl = new URL(req.url ?? '/', `http://${req.headers.host ?? 'localhost'}`);
	if (requestUrl.pathname !== endpointPath) {
		if (requestUrl.pathname === '/' || requestUrl.pathname === '/health') {
			res.writeHead(200, { 'content-type': 'application/json' });
			res.end(JSON.stringify({ ok: true, endpoint: endpointPath }));
			return;
		}
		res.writeHead(404, { 'content-type': 'application/json' });
		res.end(JSON.stringify({ error: 'Not found', endpoint: endpointPath }));
		return;
	}
	...
	res.on('close', () => {
		close().catch(error => console.error('Error closing MCP HTTP request:', error));
	});
...
function listen(httpServer: ReturnType<typeof createServer>, port: number, host: string) {
...
function parseMcpCliOptions(args: string[]): ParsedMcpCliOptions {
	const options: ParsedMcpCliOptions = {
		transport: transportFromEnv() ?? 'stdio',
		host: process.env.FHIR_CONTROLLER_MCP_HOST ?? process.env.HOST ?? defaultHttpHost,
		port: parseMcpPort(process.env.FHIR_CONTROLLER_MCP_PORT ?? process.env.MCP_PORT ?? process.env.PORT),
		path: normalizeEndpointPath(process.env.FHIR_CONTROLLER_MCP_PATH ?? defaultHttpPath),
		help: false,
	};
	...
	for (let i = 0; i < args.length; i++) {
		...
		if (arg === '--help' || arg === '-h') {
			...
			options.transport = parseTransport(nextArg(args, ++i, arg));
		} else if (arg.startsWith('--transport=')) {
			options.transport = parseTransport(arg.slice('--transport='.length));
		} else if (arg === '--host') {
			options.host = nextArg(args, ++i, arg);
		} else if (arg.startsWith('--host=')) {
			options.host = arg.slice('--host='.length);
		} else if (arg === '--port') {
			options.port = parseMcpPort(nextArg(args, ++i, arg));
		} else if (arg.startsWith('--port=')) {
			options.port = parseMcpPort(arg.slice('--port='.length));
		} else if (arg === '--path') {
			options.path = normalizeEndpointPath(nextArg(args, ++i, arg));
		} else if (arg.startsWith('--path=')) {
			options.path = normalizeEndpointPath(arg.slice('--path='.length));
		} else {
...
function printMcpHelp() {
	console.log(`Usage: fhir-controller-mcp [options]

Run the FHIR Controller MCP server.

Options:
  --stdio                 Use stdio transport (default for this binary)
  --http                  Use Streamable HTTP transport
  --transport <type>      Transport to use: stdio or http
  --host <host>           HTTP host to bind (default: ${defaultHttpHost})
  --port <port>           HTTP port to bind (default: ${defaultHttpPort})
  --path <path>           HTTP MCP endpoint path (default: ${defaultHttpPath})
  -h, --help              Show this help

Environment:
  FHIR_CONTROLLER_MCP_TRANSPORT, MCP_TRANSPORT
  FHIR_CONTROLLER_MCP_HOST
  FHIR_CONTROLLER_MCP_PORT, MCP_PORT, PORT
  FHIR_CONTROLLER_MCP_PATH`);
}

function nextArg(args: string[], index: number, option: string) {
...
function transportFromEnv(): McpTransport | undefined {
	const value = process.env.FHIR_CONTROLLER_MCP_TRANSPORT ?? process.env.MCP_TRANSPORT;
	return value ? parseTransport(value) : undefined;
}
...
function parseTransport(value: string): McpTransport {
	...
	throw new Error(`Invalid MCP transport "${value}". Expected "stdio" or "http".`);
}
